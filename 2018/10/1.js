var input = `position=< 11153,  22033> velocity=<-1, -2>
...
function processInput() {
    rows.forEach(row => {
        ...
        if (parts[1] != undefined) {
            point.positionX = +(parts[1].substring(1, parts[1].indexOf(',')));
            point.positionY = +(parts[1].substring(parts[1].indexOf(',') + 1, parts[1].indexOf('>')));

            point.velocityX = +(parts[2].substring(1, parts[2].indexOf(',')));
            point.velocityY = +(parts[2].substring(parts[2].indexOf(',') + 1, parts[2].indexOf('>')));
            points.push(point);
        }
    });
}
...
function displayPoints() {
    for (var y = 0; y <= Math.abs(maxY - minY); y++) {
        var row = "";
        for (var x = 0; x <= Math.abs(maxX - minX); x++) {
            var found = points.some(point => point.positionX == x && point.positionY == y);
            if (found) {
                row += '#';
            } else {
                row += '.';
...
function displayPointsOnCanvas() {
    ...
    for (point of points) {
        //ctx.strokeRect( point.positionX / 1000, point.positionY / 1000, 10, 10 );
        ctx.beginPath();
        ctx.arc(point.positionX, point.positionY, 1, 0, Math.PI * 2);
        ctx.stroke();
    }
}
...
function calculatePoints() {
    ...
    //var diffX = Math.abs(0 - minX);
    //var diffY = Math.abs(0 - minY);
    var diffX = 100;
    var diffY = 100;
    // recalculate coords
    
    for (point of points) {
        point.positionX = diffX + point.positionX;
        point.positionY = diffY + point.positionY;
    }
    displayPointsOnCanvas();
}

function recalculate() {
    for (point of points) {
        point.positionX = point.positionX + point.velocityX;
        point.positionY = point.positionY + point.velocityY;
    }
}
