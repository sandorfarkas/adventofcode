...
for (sleepMap of sleepMaps) {
	...
	const guardId = a[1];
	if (guardsSumSleeps.get(guardId) == undefined) {
		guardsSumSleeps.set(guardId, sleepMap.minutes.size);
	} else {
		guardsSumSleeps.set(guardId, guardsSumSleeps.get(guardId) + sleepMap.minutes.size);
	}
}

let firstGuardId = guardsSumSleeps.keys().next().value;

let max = { guardId: firstGuardId, mins: guardsSumSleeps.get(firstGuardId) };
guardsSumSleeps.forEach(
	(mins, guardId) => {
		if (mins > max.mins) {
			max = { guardId: guardId, mins: +mins };
			console.log(max);
		}
	}
)
