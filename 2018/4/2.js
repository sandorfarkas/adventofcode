...
function getFromRawLog(rawLog) {
    let log = {};

    let dateStart = rawLog.indexOf('[') + 1;
    let dateEnd = rawLog.indexOf(']');

    let dateString = rawLog.substring(dateStart, dateEnd);
    let a = dateString.split('-');
    log.date = Date.parse(dateString);
    log.year = a[0];
    log.month = a[1];
    log.day = a[2].substring(0, 2);

    let b = dateString.split(':');
	
	log.hour = b[0].slice(-2);
	log.minute = b[1];

	start = rawLog.indexOf(']') + 2;
	log.message = rawLog.substring(start);
	return log;
}

for (let rawLog of rawLogs) {
	log = getFromRawLog(rawLog);
	logs.push(log);
}
...
for (let log of logs) {
	if (log.message.indexOf('Guard') > -1) {
    	let idStart = log.message.indexOf('#') + 1;
    	let idEnd = log.message.indexOf('begins') - 1;
    	guardId = log.message.substring(idStart, idEnd);
    }
    log.guardId = guardId;
}
...
function printLog(log) {
	console.log(`[${log.year}-${log.month}-${log.day} ${log.hour}:${log.minute}] ${log.message} ${log.guardId}`);
}

/*
let sleepMap = {
	id: 'month-day#guardId',
	minutes: new Set([1,2,3,4,55,56,57]);
};
*/
const sleepMaps = [];
for (let i = 0; i < logs.length; i++) {
	...
	if (log.message.indexOf('Guard') > -1) continue;
	let nextLog = logs[i + 1];
	//printLog(nextLog);
	
	const sleepStart = log.minute;
	const sleepEnd = nextLog.minute;
	i++;
	
	const id = `${log.month}-${log.day}#${log.guardId}`;
	let sleepMap = sleepMaps.filter(sleepMap => sleepMap.id == id)[0];
	...
	if (sleepMap == undefined) {
		...
		sleepMaps.push(sleepMap);
	}

	for (let j = sleepStart; j < sleepEnd; j++) {
		sleepMap.minutes.add(+j);
	}
	
	//console.log('sm:', sleepMap);
}
/*
MAP('123', new Map(
		  1 => 1
...
let guardsSumSleeps = new Map();
for (sleepMap of sleepMaps) {
	const a = sleepMap.id.split('#');
	const guardId = a[1];

	let minCount = guardsSumSleeps.get(guardId)
	if (minCount == undefined) {
		...
		sleepMap.minutes
			.forEach(min => {
				if (minCount.get(min) == NaN || minCount.get(min) == undefined) {
					minCount.set(min, 1);	
				} else {
					minCount.set(min, +minCount.get(min) + 1);	
				}
				
			});
	}
	guardsSumSleeps.set(guardId, minCount);
}

let firstGuardId = guardsSumSleeps.keys().next().value;

let max = { guardId: firstGuardId, minute: 0, count: 0 };
guardsSumSleeps.forEach(
	(mins, guardId) => {
		mins.forEach((count, minute) => {
			if (count > max.count) max = { guardId: guardId, minute: minute, count: count };
		});
	}
)
console.log(max);

//console.log(sleepMap, sleepMap.minutes.size);
//console.log(sleepMaps.filter(sleepMap => sleepMap.id.endsWith('2857')).length);
/*const minutesRank = new Map();
for (sleepMap of sleepMaps.filter(sleepMap => sleepMap.id.endsWith('863'))) {
	for (minute of sleepMap.minutes) {
		if (minutesRank.get(minute) == undefined) {
			minutesRank.set(minute, 1);
		} else {
			minutesRank.set(minute, minutesRank.get(minute) + 1);
		}
	}
}
console.log(minutesRank);*/