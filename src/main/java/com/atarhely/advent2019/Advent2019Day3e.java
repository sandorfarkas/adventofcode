...
//        List<Point> path2 = advent2019Day3.getPath("U7,R6,D4,L4");
        ...
        Map<Point, Integer> intersectionScores = getIntersectionScores(intersections, path1, path2);

        System.out.println(intersections.stream()
                .mapToInt(p -> Math.abs(p.getX()) + Math.abs(p.getY()))
                .min().getAsInt());

        System.out.println(intersectionScores.entrySet().stream()
                .mapToInt(entry -> entry.getValue())
                .min().getAsInt());
    }

    private static Map<Point, Integer> getIntersectionScores(List<Point> intersections, List<Point> path1, List<Point> path2) {
        Map<Point, Integer> scores = new HashMap<>();
        for (Point intersection : intersections) {
            Integer score1 = getScore(intersection, path1);
            Integer score2 = getScore(intersection, path2);

            if (scores.get(intersection) != null && scores.get(intersection) > score1 + score2) scores.put(intersection, score1+score2);
            if (scores.get(intersection) == null) scores.put(intersection, score1+score2);

        }
        return scores;
    }

    private static Integer getScore(Point intersection, List<Point> path) {
        Integer score = 0;
        for (Point point : path) {
            score++;
            if (point.getX() == intersection.getX() && point.getY() == intersection.getY()) { break; }
        }
        return score;
    }

    private List<Point> getPath(String s) {
        ...
        for (String d : directions) {
            if (d.startsWith("R")) {
                for (int i = 0; i<Integer.parseInt(d.substring(1));i++) {
                    o = new Point(o.getX() + 1, o.getY());
                    path.add(o);
                }
