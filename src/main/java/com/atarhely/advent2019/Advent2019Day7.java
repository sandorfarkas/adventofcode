...
public class Advent2019Day7 {
    public static void main(String[] args) {
        Advent2019Day7 advent2019Day5 = new Advent2019Day7();
...
//        List<Integer> memory = List.of(3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9);
        List<Integer> memory = List.of(3,8,1001,8,10,8,105,1,0,0,21,34,43,60,81,94,175,256,337,418,99999,3,9,101,2,9,9,102,4,9,9,4,9,99,3,9,102,2,9,9,4,9,99,3,9,102,4,9,9,1001,9,4,9,102,3,9,9,4,9,99,3,9,102,4,9,9,1001,9,2,9,1002,9,3,9,101,4,9,9,4,9,99,3,9,1001,9,4,9,102,2,9,9,4,9,99,3,9,102,2,9,9,4,9,3,9,1001,9,2,9,4,9,3,9,102,2,9,9,4,9,3,9,101,1,9,9,4,9,3,9,1002,9,2,9,4,9,3,9,1001,9,2,9,4,9,3,9,101,2,9,9,4,9,3,9,101,1,9,9,4,9,3,9,102,2,9,9,4,9,3,9,1002,9,2,9,4,9,99,3,9,101,2,9,9,4,9,3,9,1001,9,1,9,4,9,3,9,101,1,9,9,4,9,3,9,102,2,9,9,4,9,3,9,1002,9,2,9,4,9,3,9,1001,9,1,9,4,9,3,9,1001,9,2,9,4,9,3,9,101,1,9,9,4,9,3,9,1001,9,1,9,4,9,3,9,1001,9,2,9,4,9,99,3,9,1002,9,2,9,4,9,3,9,102,2,9,9,4,9,3,9,1002,9,2,9,4,9,3,9,102,2,9,9,4,9,3,9,1001,9,1,9,4,9,3,9,1002,9,2,9,4,9,3,9,1001,9,2,9,4,9,3,9,101,1,9,9,4,9,3,9,101,2,9,9,4,9,3,9,1001,9,2,9,4,9,99,3,9,101,2,9,9,4,9,3,9,101,1,9,9,4,9,3,9,101,1,9,9,4,9,3,9,101,1,9,9,4,9,3,9,1001,9,1,9,4,9,3,9,1001,9,2,9,4,9,3,9,1001,9,2,9,4,9,3,9,102,2,9,9,4,9,3,9,1001,9,2,9,4,9,3,9,1002,9,2,9,4,9,99,3,9,1001,9,1,9,4,9,3,9,1001,9,1,9,4,9,3,9,1002,9,2,9,4,9,3,9,1001,9,1,9,4,9,3,9,1002,9,2,9,4,9,3,9,102,2,9,9,4,9,3,9,1001,9,2,9,4,9,3,9,1001,9,2,9,4,9,3,9,101,2,9,9,4,9,3,9,102,2,9,9,4,9,99);


        com.atarhely.com.advent.Permutation permutation = new com.atarhely.com.advent.Permutation();
        List<String> permute = permutation.permute("01234", 0, 4);
        int max = 0;
        for (String s : permute) {
            Integer inputA = Integer.valueOf(s.substring(0,1));
            Integer inputB = Integer.valueOf(s.substring(1,2));
            Integer inputC = Integer.valueOf(s.substring(2,3));
            Integer inputD = Integer.valueOf(s.substring(3,4));
            Integer inputE = Integer.valueOf(s.substring(4,5));
            Inputs inputs = new Inputs(inputA, inputB, inputC, inputD, inputE);
            Integer out = advent2019Day5.runAccelator(inputs, memory);
            if (out > max) {
                max = out;
    ...
    public Integer runAccelator(Inputs inputs, List<Integer> memory) {
        Integer outputA = run(memory, inputs.inputA, 0);
        Integer outputB = run(memory, inputs.inputB, outputA);
        Integer outputC = run(memory, inputs.inputC, outputB);
        Integer outputD = run(memory, inputs.inputD, outputC);
        return run(memory, inputs.inputE, outputD);
    }

    public Integer run(List<Integer> originalMemory, Integer input1, Integer input2) {
        boolean isInput1Used = false;
        Integer output = -1;
        ...
        for (int pc = 0; pc < memory.size(); ) {
            ...
            if (command.getCommand() == 99) return output;
            ...
            if (command.getCommand() == 1) {
                ...
                if (isInput1Used == false) {
                    memory.set(param1, input1);
                    isInput1Used = true;
                } else {
                    memory.set(param1, input2);
                }
                pc = pc + 2;
            } else if (command.getCommand() == 4) {
//                System.out.println(getValue(param1, command.getParamMode1(), memory));
                output = getValue(param1, command.getParamMode1(), memory);
                pc = pc + 2;
    ...
    public static class Inputs {
        ...
        private Integer inputC;
        private Integer inputD;
        private Integer inputE;
    }
}
